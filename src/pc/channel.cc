...
bool VoiceChannel::SetRemoteContent_w(const MediaContentDescription* content,
  ...
  bool is_remote_send = (audio->direction() == webrtc::RtpTransceiverDirection::kSendOnly ||
                         audio->direction() == webrtc::RtpTransceiverDirection::kSendRecv);
  bool is_my_final_recv = (type == webrtc::SdpType::kAnswer) && is_remote_send;

  RTC_LOG(LS_INFO) << "Setting remote voice description, SdpType:" << SdpTypeToString(type) << ", "
                   << "direction:" << audio->direction() << ", is_remote_send:" << is_remote_send << ", "
                   << "is_my_final_recv: " << is_my_final_recv;
  if (is_my_final_recv) {
    AudioRecvParameters recv_params;
    RtpParametersFromMediaDescription(audio, rtp_header_extensions, true, &recv_params);
    bool has_latm = false;
    for (auto& elem : recv_params.codecs) {
      if (absl::EqualsIgnoreCase(elem.name, kAacLatmCodecName)) {
        has_latm = true;
      }

      std::ostringstream oss;
      oss << "[AAC][RemoteContent]id:" << elem.id << ", name:" << elem.name << ", "
          << "clockrate:" << elem.clockrate << ", {";
    ...
    if (has_latm && !media_channel()->SetRecvParameters(recv_params, true)) {
      SafeSetError("Failed to SetRemoteContent_w with recv parameters for m-section "
                   "with mid='" + content_name() + "'.",
                    error_desc);
      return false;
    }
  }

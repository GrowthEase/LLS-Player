...
void ChannelReceive::OnReceivedPayloadData(
  ...
  int i = 0;
  for(auto it = nack_list.begin(); it != nack_list.end(); it++,i++) {
    if (i == 0) {
      ss << "SendNack2 " << *it << ",";
    } else {
      ss << " " << *it << ",";
...
void ChannelReceive::GetLatestDecodedAudioFrameTimestamp(int64_t& timestamp_ms,
                                                     int64_t& decode_clock_ms) {
  acm_receiver_.GetLatestDecodedAudioFrameTimestamp(timestamp_ms, decode_clock_ms);
}

int ChannelReceive::CurrentDelayMs() {
  return acm_receiver_.CurrentDelayMs();
}

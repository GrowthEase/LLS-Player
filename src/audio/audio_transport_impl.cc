    ...
    : audio_processing_(audio_processing),
      async_audio_processing_(
          async_audio_processing_factory
              ...
              : nullptr),
      timer_(rtc::Timer::CreateTimer(10000)),
      mixer_(mixer) {
  RTC_DCHECK(mixer);

  playing_ = true;
  timer_->start();
  decode_thread_ = rtc::PlatformThread::SpawnJoinable(
    [this]() {
      while (Decode());
    },
    "DecodingThread",
...
bool AudioTransportImpl::Decode() {
  timer_->WaitUntilNext();
  mixer_->Mix(1, &mixed_frame_);
  return playing_;
}

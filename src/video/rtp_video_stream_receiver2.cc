...
void RtpVideoStreamReceiver2::OnReceivedPayloadData(
  ...
  if(video_header.codec != kVideoCodecH264) {
    video_header.is_real_first_packet_in_frame = video_header.is_first_packet_in_frame;
  }
  ...
  // set first rtp seq num
  if (nack_module_ && !first_seq_num_set_) {
    uint16_t first_seq = packet->seq_num;
    nack_module_->OnReceivedFirstSequence(first_seq);
    first_seq_num_set_ = true;
  }

...
void VideoReceiveStream2::SetSync(Syncable* audio_syncable) {
  RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
  audio_receive_stream_ = (AudioReceiveStream*)audio_syncable;
...
void VideoReceiveStream2::StartNextDecode() {
  ...
  int64_t audio_timestamp_ms = 0;
  int64_t audio_decode_clock_ms = 0;
  if (audio_receive_stream_ != nullptr) {
    audio_receive_stream_->GetLatestDecodedAudioFrameTimestamp(audio_timestamp_ms, audio_decode_clock_ms);
    timing_->SetLatestDecodedAudioTimestamp(audio_timestamp_ms, audio_decode_clock_ms);
  }
...
int VideoReceiveStream2::CurrentDelayMs() {
  return (int)frame_buffer_->GetBufferedFramesTimeMs();
}

...
bool UnhandledPacketsBuffer::MaybeBackfillPackets(
    std::function<bool(int64_t, rtc::CopyOnWriteBuffer)> consumer) {
  size_t start;
  if (buffer_.size() < kMaxStashedPackets) {
    start = 0;
  } else {
  ...
  for (size_t i = 0; i < buffer_.size(); ++i) {
    const size_t pos = (i + start) % kMaxStashedPackets;
    if(false == consumer(buffer_[pos].packet_time_us, buffer_[pos].packet)) {

      //when that is failed firstly, return and try next
      if(first) {
        return false;
      } else {
        RTC_LOG_T(LS_WARNING)<< "UnhandledPacketsBuffer MaybeBackfillPackets fail and discard, ssrc="<<buffer_[pos].ssrc;
      }
    }
    first = false;
  }

  //all packet is Backfilled, clear the buffer
  buffer_.clear();
  insert_pos_ = 0;
  return true;
}

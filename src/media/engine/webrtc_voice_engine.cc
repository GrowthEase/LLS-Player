...
// Set and adjust gain control options.
  ...
  if (options.audio_jitter_buffer_max_packets) {
    ...
    audio_jitter_buffer_max_packets_ =
        std::max(15000, *options.audio_jitter_buffer_max_packets);
  }
...
bool WebRtcVoiceMediaChannel::SetRecvParameters(
    const AudioRecvParameters& params, bool update) {
  ...
  //update is only for codec now
  //so non-update case should keep the original code routine
  if (update) {
    if (!UpdateRecvCodecs(params.codecs)) {
      return false;
    }
  } else {
...
bool WebRtcVoiceMediaChannel::UpdateRecvCodecs(const std::vector<AudioCodec>& codecs) {
  RTC_DCHECK_RUN_ON(worker_thread_);

  // Set the payload types to be used for incoming media.
  RTC_LOG(LS_INFO) << "SettingUpdateRecvCodecs()";
  ...
  for (const AudioCodec& codec : codecs) {
    ...
    // treated as an error. It's abnormal, but not really illegal.
    AudioCodec old_codec;
    bool found = FindCodec(recv_codecs_, codec, &old_codec);
    if (found) {
      if (old_codec.id != codec.id) {
        RTC_LOG(LS_WARNING) << "WebRtcVoiceMediaChannel::UpdateRecvCodecs() "
          << codec.name << " mapped to a second payload type ("
          << codec.id << ", was already mapped to "
          << old_codec.id << ")";
      }
      //TO IMPLEMENT, emplment for more param:
      //it's only for latm-liked codec, which parameter will be specified by answer
      //and up caller will limit calling to latm
      for (AudioCodec& c : recv_codecs_) {
        if (c.Matches(codec)) {
          for (auto &coming_param : codec.params) {
            c.params.insert( {coming_param.first, coming_param.second});            
          }
        }
      }
    }
    else {
      recv_codecs_.push_back(codec);    
    }
    ...
    if (!IsCodec(codec, kCnCodecName) && !IsCodec(codec, kDtmfCodecName) &&
       ...
       !engine()->decoder_factory_->IsSupportedDecoder(format)) {
      RTC_LOG(LS_ERROR) << "UpdateRecvCodecs() Unsupported codec: " << rtc::ToString(format);
      return false;
    }
    // We allow adding new codecs but don't allow changing the payload type of
    // codecs that are already configured since we might already be receiving
    ...
    // Though this check really should happen at a higher level, since this
    // conflict could happen between audio and video codecs.
    auto dec_map_it = decoder_map_.find(codec.id);
    bool already_in_dec_map = (dec_map_it != decoder_map_.end());

    if (already_in_dec_map) {
      if (!dec_map_it->second.Matches(format)) {
        RTC_LOG(LS_ERROR) << "UpdateRecvCodecs() Attempting to use payload type " << codec.id
          << " for " << codec.name
          << ", but it is already used for "
          << dec_map_it->second.name;
        return false;      
      }

      //update the sdp codec-param in dec-map 
      for (auto &param : codec.params) {
        //put all the codec-params into SdpAudioFormat-param in dec-map
        dec_map_it->second.parameters.insert({param.first, param.second});      
      }
    }
    else {
      decoder_map_.insert({ codec.id, std::move(format) });
    }

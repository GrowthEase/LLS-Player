...
RtpReceiveStats StreamStatisticianImpl::GetStats() const {
  ...
  absl::optional<int> lost_rate = const_cast<StreamStatisticianImpl*>(this)->GetPeriodFractionLostInPercent();// GetFractionLostInPercent();
  if (lost_rate.has_value()) {
    stats.fraction_lost = lost_rate.value();
  }
...
absl::optional<int> StreamStatisticianImpl::GetPeriodFractionLostInPercent() {
  // Calculate fraction lost.
  int64_t exp_since_last = received_seq_max_ - last_seq_max_;
  RTC_DCHECK_GE(exp_since_last, 0);

  int32_t lost_since_last = cumulative_loss_ - last_cumulative_loss_;
  if (exp_since_last <= 0) {
    return absl::nullopt;
  }
  ...
  last_cumulative_loss_ = cumulative_loss_;
  last_seq_max_ = received_seq_max_;
  return 100 * static_cast<int64_t>(lost_since_last) / exp_since_last;
}

...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuaNalu(
  ...
  bool is_real_first_packet_in_frame = false;
  NaluInfo nalu;
  ...
  if (first_fragment) {
    ...
    if(original_nal_type == H264::NaluType::kIdr 
      ...
      is_real_first_packet_in_frame = H264BitstreamParser::IsFirstSliceInFrame(rtp_payload.cdata(), rtp_payload.size());
    } else {
      is_real_first_packet_in_frame = true;
    }
  ...
  parsed_payload->video_header.is_real_first_packet_in_frame = is_real_first_packet_in_frame;
  auto& h264_header = parsed_payload->video_header.video_type_header
                          .emplace<RTPVideoHeaderH264>();

...
PacketBuffer::InsertResult PacketBuffer::InsertPacket(
  ...
  if (!first_packet_received_) {
    ...
    RTC_LOG(LS_INFO) << "first video packet received, seq:" << packet->seq_num << " timestamp:" << packet->timestamp/90 << " pt:" << packet->payload_type;
...
bool PacketBuffer::PotentialNewFrame(uint16_t seq_num) const {
  ...
  if (entry->seq_num != seq_num)
    return false;
  if (entry->is_first_packet_in_frame() && entry->is_real_first_packet_in_frame())
    return true;

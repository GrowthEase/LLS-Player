...
const int kDefaultRttMs = 40;
const int kMaxNackRetries = 20;
...
void NackRequester::UpdateRtt(int64_t rtt_ms) {
  RTC_DCHECK_RUN_ON(worker_thread_);
  rtt_ms_ = rtt_ms < kDefaultRttMs ? kDefaultRttMs : rtt_ms;
}

void NackRequester::OnReceivedFirstSequence(uint16_t first_seq) {
  RTC_DCHECK_RUN_ON(worker_thread_);
  if (!first_seq_num_received_) {
    uint16_t seq = first_seq > 10 ? first_seq-10 : 0;
    OnReceivedPacket(seq, false);
    first_seq_num_received_ = true;
  }
}

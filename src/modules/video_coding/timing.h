...
class VCMTiming {
 public:
  ...
  void SetLatestDecodedAudioTimestamp(int64_t timestamp, int64_t decode_clock_ms);

  enum { kDefaultRenderDelayMs = 10 };
  enum { kDelayMaxChangeMsPerS = 100 };
  enum { kDefaultAVSyncThresholdMs = 10000 };
  enum { kDefaultVideoWaitTimeMs = 25 };

 protected:
  int RequiredDecodeTimeMs() const RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  int64_t RenderTimeMsInternal(uint32_t frame_timestamp, int64_t now_ms) const
      RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  int64_t RenderTimeMsBaseAudio(uint32_t frame_timestamp, int64_t now_ms) const
      RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);
 ...
 private:
  ...
  int64_t latest_audio_decode_ts_ RTC_GUARDED_BY(mutex_);
  int64_t latest_audio_decode_clock_ms_ RTC_GUARDED_BY(mutex_);
  int64_t sync_threshold_ RTC_GUARDED_BY(mutex_);

  rtc::TimestampWrapAroundHandler timestamp_wraparound_handler_;
  bool first_audio_timestamp_set;
  static int64_t last_log_print_ms_;
};
}  // namespace webrtc

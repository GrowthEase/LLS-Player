...
int64_t VCMTiming::last_log_print_ms_ = 0;
...
VCMTiming::VCMTiming(Clock* clock)
    : clock_(clock),
      ...
      latest_audio_decode_ts_(0),
      latest_audio_decode_clock_ms_(0),
      sync_threshold_(kDefaultAVSyncThresholdMs),
      first_audio_timestamp_set(false) {
...
void VCMTiming::set_min_playout_delay(int min_playout_delay_ms) {
  RTC_LOG(LS_INFO) << "set_min_playout_delay, min_playout_delay_ms:" << min_playout_delay_ms;
  MutexLock lock(&mutex_);
...
int64_t VCMTiming::RenderTimeMs(uint32_t frame_timestamp,
  ...
  return RenderTimeMsBaseAudio(frame_timestamp, now_ms);
}
...
void VCMTiming::SetLatestDecodedAudioTimestamp(int64_t timestamp_ms, int64_t decode_clock_ms) {
  MutexLock lock(&mutex_);
  if (!first_audio_timestamp_set && timestamp_ms > 0) {
    RTC_LOG(LS_INFO) << "SetLatestDecodedAudioTimestamp, first_audio_timestamp_ms:" << timestamp_ms << " decode_clock_ms:" << decode_clock_ms;
    first_audio_timestamp_set = true;
  }
  latest_audio_decode_ts_ = timestamp_ms;
  latest_audio_decode_clock_ms_ = decode_clock_ms;
}
...
int64_t VCMTiming::RenderTimeMsBaseAudio(uint32_t frame_timestamp,
                                         int64_t now_ms) const {
 rtc::TimestampWrapAroundHandler* hander = const_cast<rtc::TimestampWrapAroundHandler*>(&timestamp_wraparound_handler_);
 int64_t timestamp_wrapped = hander->Unwrap(frame_timestamp);
 constexpr int kLowLatencyRendererMaxPlayoutDelayMs = 500;
 if (min_playout_delay_ms_ == 0 &&
      (max_playout_delay_ms_ == 0 ||
       (low_latency_renderer_enabled_ &&
  ...
  if (latest_audio_decode_clock_ms_ == 0 || latest_audio_decode_ts_ == 0) {
    RTC_LOG(LS_INFO) << "[Timing] latest_audio_decode_clock_ms and latest_audio_decode_ts both zero";
    return RenderTimeMsInternal(frame_timestamp, now_ms);
  }

  int64_t timestamp_offset = timestamp_wrapped / 90 - latest_audio_decode_ts_;
  if (sync_threshold_ > 0 && abs(timestamp_offset) > sync_threshold_) {
    if (timestamp_offset < -sync_threshold_) {
      timestamp_offset = kDefaultVideoWaitTimeMs;
    } else {
      timestamp_offset = -1;
    }
  }
  ...
  if (now - last_log_print_ms_ >= 5000) {
    RTC_LOG(LS_INFO) << "[Timing] timestamp_offset: " << timestamp_offset << " decode_clock_ms:" << latest_audio_decode_clock_ms_ << " v_ts:" << frame_timestamp/90 << " a_ts:" << latest_audio_decode_ts_;
    last_log_print_ms_ = now;
  }
  return latest_audio_decode_clock_ms_ + timestamp_offset;
}

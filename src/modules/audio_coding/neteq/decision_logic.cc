...
constexpr int kMinFastModeThreshold = 11;
...
DecisionLogic::DecisionLogic(
    ...
    : delay_manager_(std::move(delay_manager)),
      ...
      target_level_window_ms_("target_level_window",
                              ...
                              0,
                              absl::nullopt),
      fast_mode_threshold_(kMinFastModeThreshold) {
...
NetEq::Operation DecisionLogic::GetDecision(const NetEqStatus& status,
  ...
  int64_t ts_diff  = (int64_t)status.next_packet->timestamp - (int64_t)status.target_timestamp;
  // RTC_LOG(LS_INFO) << "555555 next_packet->timestamp:" << status.next_packet->timestamp << " target_timestamp:" << status.target_timestamp << " ts_diff:" << ts_diff;
  //if (status.target_timestamp == status.next_packet->timestamp) {
  if (abs((int)ts_diff) < 50) {
...
NetEq::Operation DecisionLogic::ExpectedPacketAvailable(NetEq::Mode prev_mode,
  ...
  if (!disallow_time_stretching_ && prev_mode != NetEq::Mode::kExpand &&
    ...
    if (buffer_level_samples >= high_limit * fast_mode_threshold_ / 10.0)

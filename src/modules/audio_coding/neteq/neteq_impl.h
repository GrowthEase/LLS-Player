...
class NetEqImpl : public webrtc::NetEq {
 public:
  ...
  void GetLatestDecodedAudioFrameTimestamp(int64_t& timestamp_ms, int64_t& decode_clock_ms) override;
 ...
 protected:
  ...
  void SetLatestAudioDecodedTimestamp(int64_t timestamp_rtp)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);

  inline bool ValidateLatm(uint8_t* data, uint32_t size, 
                    uint32_t &sample_rate, uint32_t &channels)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(mutex_);

  int32_t CalcRtpTimestampInterval(int32_t seq, int32_t timestamp_rtp);

  /*
   * class BitStreamReader is for stream mux config for big-endian
   */
  class BitStreamReader {
   public:
    ...
    bool Init(uint8_t* data, int max_bytes);
    //inline 
    bool Read(int n/*bits*/, uint32_t& value, std::string& desc);
    //inline 
    bool Skip(int n/*bits*/, std::string& desc);
    void Uninit();
  ...
  class SMC {
   public:
    ...
    void Update(uint32_t use_same_stream_mux,
    ...
    bool Equals(uint32_t use_same_stream_mux,
      uint32_t audio_mux_version, uint32_t allStreamsSameTimeFraming,
      uint32_t num_sub_frames, uint32_t num_program,
      uint32_t num_layer, uint32_t asc_auido_object_type,
      uint32_t sr_idx, uint32_t channel_cfg);
    ...
    virtual ~SMC() = default;
   private:
    uint32_t use_same_stream_mux_ = -1;
    uint32_t audio_mux_version_   = -1;
    uint32_t allStreamsSameTimeFraming_ = -1;
    uint32_t num_sub_frames_ = -1;
    uint32_t num_program_    = -1;
    uint32_t num_layer_      = -1;
    uint32_t asc_auido_object_type_ = -1;
    uint32_t sr_idx_      = -1;
    uint32_t channel_cfg_ = -1;
  };
  ...
  absl::optional<int> delayed_last_output_sample_rate_hz_
      RTC_GUARDED_BY(mutex_);
  int64_t latest_decoded_timestamp_rtp_ RTC_GUARDED_BY(mutex_);
  int64_t latest_decoded_clock_ms_ RTC_GUARDED_BY(mutex_);
  rtc::TimestampWrapAroundHandler timestamp_wraparound_handler_;
  bool first_packet_received_;
  bool first_packet_decoded_;
  std::map<int32_t, int32_t> seq_timestamp_;
  bool set_timstamp_interval_;
  bool actual_fs_hz_set_;

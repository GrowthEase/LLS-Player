...
NetEqImpl::NetEqImpl(const NetEq::Config& config,
    ...
    : clock_(deps.clock),
      ...
      latest_decoded_timestamp_rtp_(0),
      latest_decoded_clock_ms_(0),
      first_packet_received_(false),
      first_packet_decoded_(false),
      set_timstamp_interval_(false),
      actual_fs_hz_set_(false) {
...
int NetEqImpl::InsertPacket(const RTPHeader& rtp_header,
  ...
  RTPHeader header = rtp_header;
  if (!set_timstamp_interval_) {
    int32_t interval = CalcRtpTimestampInterval(header.sequenceNumber, header.timestamp);
    if (interval != 0) {
      red_payload_splitter_->SetTimestampInterval(interval);
      seq_timestamp_.clear();
      set_timstamp_interval_ = true;
    }
  }
  ...
  if (format && absl::EqualsIgnoreCase(format->sdp_format.name, "MP4A-ADTS")) {
    ...
    if (payload[0] == 0xff && ((payload[1] & 0xf0) == 0xf0)) {
      int size = 0;
      size |= ((payload[3] & 0x03) << 11);
      size |= payload[4] << 3;
      size |= ((payload[5] & 0xe0) >> 5);
      uint8_t sampling_frequency_index = (payload[2] & 0x3c) >> 2;
      int rtp_sample_rate = audio_sample_rates[sampling_frequency_index];
      //RTC_LOG(LS_INFO) << "wbi NetEqImpl::InsertPacketInternal samplerate:" << rtp_sample_rate << " sampling_frequency_index:" << sampling_frequency_index << " size:" << size << " payload_size:" << payload.size();
      if (rtp_sample_rate != 48000) {
        uint32_t tmp = ((uint64_t)header.timestamp*48000)/rtp_sample_rate;
        header.timestamp = tmp;
        if (!actual_fs_hz_set_) {
          red_payload_splitter_->SetActualClockRate(rtp_sample_rate);
          actual_fs_hz_set_ = true;
        }
        //RTC_LOG(LS_INFO) << "wbi NetEqImpl::InsertPacketInternal rewrite timestamp:" << header.timestamp << " ori ts:" << rtp_header.timestamp;
      }
    } else {
      // not adts aac frame
    }
  ...
  if (format && absl::EqualsIgnoreCase(format->sdp_format.name, "MP4A-LATM")) {
    uint32_t rtp_sample_rate = 48000;
    uint32_t channels = 2;

    if (format->sdp_format.IsCPresented()) {
      if (!ValidateLatm(const_cast<uint8_t*>(payload.data()), payload.size(), rtp_sample_rate, channels)) {
        return kFail;
      }    
    }
    else {
      ...
      bool is_ok = format->sdp_format.GetInfoFromConfig(channels, rtp_sample_rate, 
                                                        sbr_enabled, ps_enabled, extra_data);
      if (!is_ok)
        return kFail;     
    }

    if (!rtp_sample_rate) {
      RTC_LOG(LS_ERROR) << "[LATM] ValidateLatm() find the rtp_sample_rate=0";
      return kFail;
    }
  ...
  if (!first_packet_received_) {
    ...
    RTC_LOG(LS_INFO) << "first audio packet received, seq:" << header.sequenceNumber << " timestamp:" << header.timestamp/(fs_hz_/1000) << " pt:" << header.payloadType << " ssrc:" << header.ssrc ;
  }
  if (InsertPacketInternal(header, payload) != 0) {
    return kFail;
  }
  return kOK;
}
...
int NetEqImpl::GetAudio(AudioFrame* audio_frame,
  ...
  int64_t timestamp_wrap = timestamp_wraparound_handler_.Unwrap(audio_frame->timestamp_);
  if (!first_packet_decoded_) {
    RTC_LOG(LS_INFO) << "first audio decode rtp_ts:" << audio_frame->timestamp_ << " ts_wrap:" << timestamp_wrap;
    first_packet_decoded_ = true;
  }
  SetLatestAudioDecodedTimestamp(timestamp_wrap);
  ...
  auto decoder = decoder_database_->GetActiveDecoder();
  if (decoder != nullptr) {
    decoder->OnAudioFrame(audio_frame);
  }
...
int NetEqImpl::InsertPacketInternal(const RTPHeader& rtp_header,
  ...
  if (nack_enabled_) {
    ...
    while (it != packet_list.end()) {
      ...
      uint32_t timestamp = current_packet.timestamp;
      uint16_t sequence_number = current_packet.sequence_number;
      nack_->UpdateLastReceivedPacket(sequence_number, timestamp);
      ++it;
    }
  }
...
int NetEqImpl::GetDecision(Operation* operation,
  ...
  *operation = Operation::kUndefined;
  ...
  if (!new_codec_) {
    ...
    packet_buffer_->DiscardOldPackets(end_timestamp-50, five_seconds_samples,
                                      stats_.get());
  }
...
void NetEqImpl::GetLatestDecodedAudioFrameTimestamp(int64_t& timestamp_ms, int64_t& decode_clock_ms) {
  MutexLock lock(&mutex_);
  timestamp_ms = latest_decoded_timestamp_rtp_ * 1000 / fs_hz_;
  decode_clock_ms = latest_decoded_clock_ms_;
}

void NetEqImpl::SetLatestAudioDecodedTimestamp(int64_t timestamp_rtp) {
  latest_decoded_timestamp_rtp_ = timestamp_rtp;
  latest_decoded_clock_ms_ = clock_->TimeInMilliseconds(); 
}

int32_t NetEqImpl::CalcRtpTimestampInterval(int32_t seq, int32_t timestamp_rtp) {
  if (seq_timestamp_.size() < 2) {
    seq_timestamp_.insert(std::pair<int32_t, int32_t>(seq, timestamp_rtp));
  } else {
    auto it = seq_timestamp_.begin();
    int32_t seq1 = it->first;
    int32_t timestamp1 = it->second;
    it++;
    int32_t seq2 = it->first;
    int32_t timestamp2 = it->second;
    int32_t interval = abs(timestamp2 - timestamp1)/abs(seq2-seq1);
    RTC_LOG(LS_INFO) << "NetEqImpl::CalcRtpTimestampInterval interval:" << interval;
    return interval;
  }
  return 0;
}
...
bool NetEqImpl::ValidateLatm(uint8_t* data, uint32_t size, 
                             uint32_t& sample_rate, uint32_t &channels) {
  BitStreamReader reader;
  std::string desc;

  if (!reader.Init(data, size)) {
    return kFail;
  }

  //for LOAS header:
  uint32_t loas_sync_word = 0;
  desc = "loasSycWord";
  if (!reader.Read(11, loas_sync_word, desc)) {
    return false;
  }
  if (LOAS_SYNC_WORD != loas_sync_word) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err loas_sync_word(" << loas_sync_word << ") != 0x2b7";
    return false;
  }

  uint32_t loas_frame_length = 0;
  desc = "loasFrameLenth";
  if (!reader.Read(13, loas_frame_length, desc)) {
    return false;
  }

  //for AudioMuxElement
  uint32_t use_same_stream_mux = 0;
  desc = "useSameStreamMux";
  if (!reader.Read(1, use_same_stream_mux, desc)) {
    return false;
  }
  if (0 != use_same_stream_mux) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err use_same_stream_mux("<< use_same_stream_mux <<")!=0";
    return false;
  }

  desc = "audioMuxVersion";
  uint32_t audio_mux_version = 0;
  if (!reader.Read(1, audio_mux_version, desc)) {
    return false;
  }
  if (0 != audio_mux_version) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err audio_mux_version("<< audio_mux_version <<") !=0";
    //if support(1 == audio_mux_version), do something more on audio_mux_versionA
    return false;
  }

  //desc = "audioMuxVersionA";
  //if (!reader.Skip(1, desc)) {
  //  return false;
  //}
  desc = "allStreamsSameTimeFraming";
  uint32_t all_stream_same_time_framing = 0;
  if (!reader.Read(1, all_stream_same_time_framing, desc)) {
    return false; //no more value check
  }

  desc = "numSubFrames";
  uint32_t num_sub_frames;
  if (!reader.Read(6, num_sub_frames, desc)) {
    return false;
  }
  if (0 != num_sub_frames) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err num_sub_frames("<< num_sub_frames <<")!=0" ;
    return false;
  }

  desc = "numProgram";
  uint32_t num_program;
  if (!reader.Read(4, num_program, desc)) {
    return false;
  }
  if (0 != num_program) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err num_program("<< num_program <<")!=0";
    return false;
  }

  desc = "numLayer";
  uint32_t num_layer;
  if (!reader.Read(3, num_layer, desc)) {
    return false;
  }
  if (0 != num_layer) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err num_layer("<< num_layer <<")!=0";
    return false;
  }
  ...
  if (!reader.Read(5, asc_auido_object_type, desc)) {
    return false;
  }
  if (asc_auido_object_type >= 31) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err asc_auido_object_type("<< asc_auido_object_type <<") >= 31";
    return false;
  }

  desc = "asc_samplingFrequencyIndex";
  uint32_t sr_idx;
  if (!reader.Read(4, sr_idx, desc)) {
    return false;  
  }
  if (sr_idx >= 0xC) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation err on asc_samplingFrequencyIndex(" << sr_idx << ") >= 0xC";
    return false;
  }

  sample_rate = audio_sample_rates[sr_idx];
  ...
  if (!reader.Read(4, channel_cfg, desc)) {
    return false;
  }
  if (2 != channel_cfg && 1 != channel_cfg) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM] Validation on asc_channelConfiguration(" << channel_cfg << ") != 1 or 2";
    return false;
  }
  channels = channel_cfg * ((29 == asc_auido_object_type) ? 2 : 1);

  //TODO MORE: optimize log output by param-changed
  if (false == smc_.Equals(use_same_stream_mux,
                           audio_mux_version, all_stream_same_time_framing,
                           num_sub_frames, num_program,
                           num_layer, asc_auido_object_type,
                           sr_idx, channel_cfg)) {
      RTC_LOG(LS_INFO) << "[AAC][LATM][NEQ]ValidateLatm param -- "
                       << "use_same_stream_mux(" << use_same_stream_mux << "), "
                       << "audio_mux_version(" << audio_mux_version << "), "
                       << "allStreamsSameTimeFraming("<< all_stream_same_time_framing <<")-no-value-check, "
                      ...
                      //  << "use_same_config(" << use_same_config << "), "
                       ...
                       << "sr_idx(" << sr_idx << ")-" << sample_rate << "kHz, "
                       << "channel_cfg(" << channel_cfg << ")-" << channels << "ch(s), ";

      smc_.Update(use_same_stream_mux,
                  audio_mux_version, all_stream_same_time_framing,
                  num_sub_frames, num_program,
                  num_layer, asc_auido_object_type,
                  sr_idx, channel_cfg);
...
bool NetEqImpl::BitStreamReader::Init(uint8_t* data, int max_bytes) {

  if (!data || max_bytes <= 0 || max_bytes >= 1500) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM]BitStreamReader::Init() err. data_pointer:" << data << ", "
      << "max_bytes:" << max_bytes;
    return false;
  }
...
//inline  
bool NetEqImpl::BitStreamReader::Read(int n, uint32_t &value, std::string& desc) {
  if (n < 0 || n >= 25) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM]BitStreamReader::Read() err. "
                      << "desc(" << desc << ") "
                      << "n(" << n << ") is out of limitation: [0, 24]";
    return false;
  }

  if (n + offset_ > max_bits_) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM]BitStreamReader::Read() err. "
                      << "desc(" << desc << ") "
                      << "no more for bits(" << n << "), "
                      << "max_bits_:" << max_bits_ << ", cur_offset:" << offset_;
    return false;
  }

  uint8_t* x = data_ + (offset_ >> 3);

  //TO IMPLEMENT:
  uint32_t temp = (((uint32_t)x[0]) << 24) | (x[1] << 16) | (x[2] << 8) | (x[3]);  
  temp = temp << (offset_ & 0x7);
...
inline bool NetEqImpl::BitStreamReader::Skip(int n/*bits*/, std::string& desc) {
  if (n < 0 || n >= 25) {
    RTC_LOG(LS_ERROR) << "[AAC][LATM]BitStreamReader::Skip() err. "
                      << "desc(" << desc << ") "
                      << "n(" << n << ") is out of limitation: [0, 24]";
    return false;
  }
...
void NetEqImpl::BitStreamReader::Uninit() {
  data_   = nullptr;
  offset_ = 0;
  max_bytes_ = 0;
  max_bits_   = 0;
...
void NetEqImpl::SMC::Update(uint32_t use_same_stream_mux,
                            uint32_t audio_mux_version, uint32_t allStreamsSameTimeFraming,
                            uint32_t num_sub_frames, uint32_t num_program,
                            uint32_t num_layer, uint32_t asc_auido_object_type,
                            uint32_t sr_idx, uint32_t channel_cfg) {
    use_same_stream_mux_ = use_same_stream_mux;
    audio_mux_version_   = audio_mux_version;

    allStreamsSameTimeFraming_ = allStreamsSameTimeFraming;

    num_sub_frames_ = num_sub_frames; 
    num_program_    = num_program;
    num_layer_      = num_layer; 

    asc_auido_object_type_ = asc_auido_object_type;

    sr_idx_      = sr_idx; 
    channel_cfg_ = channel_cfg;
}

bool NetEqImpl::SMC::Equals(uint32_t use_same_stream_mux,
  uint32_t audio_mux_version, uint32_t allStreamsSameTimeFraming,
  uint32_t num_sub_frames, uint32_t num_program,
  uint32_t num_layer, uint32_t asc_auido_object_type,
  uint32_t sr_idx, uint32_t channel_cfg) {
  if (use_same_stream_mux != use_same_stream_mux_ ||
      audio_mux_version != audio_mux_version_ ||
      allStreamsSameTimeFraming != allStreamsSameTimeFraming_) {
    return false;
  }

  if (num_sub_frames != num_sub_frames_ ||
      num_program != num_program_ ||
      num_layer != num_layer_ ||
      asc_auido_object_type != asc_auido_object_type_ ||
      sr_idx != sr_idx_ || 
      channel_cfg != channel_cfg_) {
    return false;
  }    

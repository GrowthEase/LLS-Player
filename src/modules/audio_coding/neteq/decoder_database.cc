...
DecoderDatabase::DecoderInfo::DecoderInfo(
  ...
  if (absl::EqualsIgnoreCase(audio_format.name, "MP4A-LATM") 
      && 48000 != audio_format_.clockrate_hz) {
    SdpAudioFormat& dummy = const_cast<SdpAudioFormat&>(audio_format_);
    dummy.clockrate_hz = 48000;
  }

  RTC_LOG(LS_INFO) << "[AAC][DecoderInfo]name:" << audio_format.name << ", "
                   << "ch:" << audio_format_.num_channels << ", "
                   << "input-clock-hz:" << audio_format.clockrate_hz << ", "
                   << "output-clock-hz:" << audio_format_.clockrate_hz;
}

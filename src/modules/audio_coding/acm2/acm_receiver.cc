...
void AcmReceiver::GetLatestDecodedAudioFrameTimestamp(int64_t& timestamp_ms,
                                                  int64_t& decode_clock_ms) {
  neteq_->GetLatestDecodedAudioFrameTimestamp(timestamp_ms, decode_clock_ms);
}

int AcmReceiver::CurrentDelayMs() {
  return neteq_->FilteredCurrentDelayMs();
}
